---
title: Queue
description: In-process background job queue with retry, dead-lettering, deduplication, and delayed execution.
category: guide
since: 2.0.0
status: stable
crate: tako-rs-core
...
[API stability](/docs/reference/stability). The queue integrates with
the in-process [signals](/docs/signals) bus, so any listener can react
