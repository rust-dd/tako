...
- ~~`tako-core-local` (the `!Send` router) is missing plugins, signals, OpenAPI, timeout, fallback, TSR, error_handler, and `mount_all`. Either reach parity, or document the trade-off explicitly and label it as a niche tool.~~ — **Done by removal.** `tako-core-local`, `tako-server-pt::serve_per_thread_local` and `serve_per_thread_compio_local`, plus the `per-thread-local` / `per-thread-compio-local` features and the `tako::local` re-export module are all deleted. Per-worker isolation is fully covered by `serve_per_thread` / `serve_per_thread_compio` with the thread-safe `Router` (same per-worker Box::leak'd instance, all 26 v2 features available, `Arc<Mutex<…>>` instead of `Rc<RefCell<…>>` for shared state). The `LocalRouter` niche didn't earn the dual-maintenance cost.
